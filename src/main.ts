...
export default class SlackCleaner extends Plugin {
    ...
    async cleanData(): Promise<void> {
        ...
        const formattedMessages = cleanedData.map(msg => `> ${msg.replace(/\n/g, '\n> ')}`);
    ...
    cleanSlackText(htmlData: string): string[] {
        ...
        const messageContainers = doc.querySelectorAll('div[data-qa="message_content"]');
        ...
        messageContainers.forEach(container => {
            ...
            this.debugging && console.log('timestampElem', timestampElem?.textContent);
            ...
            const timestamp = timestampNumber ? unixTimestampToDate(timestampNumber) : timestampLinkElem?.getAttribute('aria-label')?.trim() || timestampElem?.textContent?.trim() || '';
            this.debugging && console.log('timestampLinkElem', timestampLinkElem?.getAttribute('href'));
